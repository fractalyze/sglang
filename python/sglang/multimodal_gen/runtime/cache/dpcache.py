...
"""DPCache: calibrated key-step schedules for training-free step skipping.
...
* The predictor is first order: ``h_j + (h_j - h_i) / (j - i) * (t - j)``
  ...
  and the arithmetic runs in the feature dtype (BF16 for Qwen-Image 2.1) with
  the same operation order in calibration and inference.
...
def uniform_full_steps(
    ...
    interior keys differs.
    """
    mandatory = _validate_mandatory(num_steps, mandatory)
    if not _is_int(budget) or not _is_int(last_key):
...
def _require(condition: bool, message: str) -> None:
    if not condition:
        raise ValueError(f"dpcache_schedule: {message}")
...
def validate_schedule(schedule: Mapping[str, Any]) -> tuple[int, ...]:
    ...
    if not isinstance(schedule, Mapping):
        raise TypeError("dpcache_schedule must be a JSON object")
...
def check_schedule_matches(
    ...
    if mismatched:
        ...
        raise ValueError(
            f"dpcache_schedule does not match this request ({mismatched}). {details}"
        )
    return steps
