...
class DPCacheMixin:
    ...
    timestep. ``DenoisingStage`` rejects a schedule for any other model.
    """
    ...
    def dpcache_blocks(self, run_blocks: Callable[[], torch.Tensor]) -> torch.Tensor:
        ...
        if states is None:
            if getattr(batch, "dpcache_schedule", None) and not batch.is_warmup:
                # an enabled schedule must never silently run uncached
                raise RuntimeError("dpcache_schedule is set but no state is attached")
            return run_blocks()
