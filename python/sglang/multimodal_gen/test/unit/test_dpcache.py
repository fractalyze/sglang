...
def test_schedule_artifact_roundtrip_and_request_match():
    ...
    assert SamplingParams(dpcache_schedule=schedule).dpcache_schedule
...
def test_malformed_schedules_are_rejected(overrides):
    ...
    with pytest.raises(ValueError):
        SamplingParams(dpcache_schedule=artifact(**overrides))
...
def forward_batch(states=None, schedule=None, is_cfg_negative=False, is_warmup=False):
...
@pytest.mark.parametrize(
    "batch",
    [None, forward_batch(), forward_batch(schedule={"any": 1}, is_warmup=True)],
)
...
def test_hook_refuses_an_enabled_schedule_without_state():
    """An enabled schedule must never silently run uncached."""
    dit = _TinyDiT()
    with pytest.raises(RuntimeError, match="no state is attached"):
        run_dit(dit, 0, torch.ones(1), forward_batch(schedule={"any": 1}))
...
def make_stage(dtype=torch.bfloat16, lora=None, supported=True, **server_overrides):
...
def test_stage_without_schedule_is_untouched(monkeypatch):
    ...
    monkeypatch.setattr(
        DenoisingStage,
        "_denoise",
        lambda self, batch, server_args: calls.append(batch.dpcache_states) or batch,
    )
...
def test_warmup_request_runs_without_the_schedule(monkeypatch):
    """A warmup copy keeps the schedule but runs fewer steps than it was built for."""
    stage, server_args = make_stage()
    batch = make_batch(artifact())
...
def test_stage_rejects_a_model_without_the_hook(monkeypatch):
    ...
    with pytest.raises(ValueError, match="not supported by _TinyDiT"):
        stage.forward(make_batch(artifact()), server_args)
...
def scheduler_request(schedule):
    sampling = SamplingParams(prompt="a cat", dpcache_schedule=schedule)
    return SimpleNamespace(
        is_warmup=False,
...
@pytest.mark.parametrize("schedules", [(True, True), (True, False), (False, True)])
def test_dpcache_requests_are_not_dynamically_batched(schedules):
    ...
    base, candidate = (
        scheduler_request(artifact() if on else None) for on in schedules
    )
