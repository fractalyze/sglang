...
def test_uniform_keys_match_the_prefix_budget_and_last_key(budget, last_key, expected):
    ...
    keys = comp.uniform_full_steps(40, budget, last_key)
...
def test_uniform_keys_round_half_up_and_reject_infeasible_budgets():
    ...
    assert comp.uniform_full_steps(40, 5, 12)[3] == 7
    for budget, last_key in ((3, 39), (41, 39), (12, 40), (12, 2)):
        with pytest.raises((ValueError, TypeError)):
            comp.uniform_full_steps(40, budget, last_key)
    with pytest.raises(TypeError):
        comp.uniform_full_steps(40, 12.0, 38)


def test_uniform_schedules_are_not_part_of_the_served_runtime():
    """The control lives in the comparator; the upstream cache ships no generator."""
    from sglang.multimodal_gen.runtime.cache import dpcache

    assert not hasattr(dpcache, "uniform_full_steps")
    assert not hasattr(dpcache, "UNIFORM_GENERATOR")
    # it still travels as ordinary schedule_method provenance
    assert comp.UNIFORM_GENERATOR == "endpoint-matched-uniform-v1"
